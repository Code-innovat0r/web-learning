...
const Manager = () => {
    ...
    const savePassword = async () => {
        ...
        await fetch("http://127.0.0.1:3000/", {method: "POST", headers: {'Content-Type': 'application/json'}, body: JSON.stringify({...form, id: a})})
    ...
    const deleteRow = async (id, boolean = true) => {
        if (boolean) {
            let conf = confirm("Are you sure you want to delete ??")
            if (conf) {
                setformarray(formarray.filter(i => i.id !== id))
                let res = await fetch("http://127.0.0.1:3000/", {method: "DELETE", headers: {'Content-Type': 'application/json'}, body: JSON.stringify({id})})
    ...
    const editRow = (id) => {
        let con = confirm("Make sure that you must save After edit... pre-existance is deleted")
        if(con){
